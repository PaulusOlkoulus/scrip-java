...
async function fillModal(modal) {
    ...
    modal.addEventListener("show.bs.modal", async function(event) {
        ...
        if (rolesSelectDelete !== null) {
            ...
            userRolesHTML += `
                <option value="ROLE_USER" ${user.roleList.some(role => role.role === 'ROLE_USER') ? 'selected' : ''}>USER</option>
                <option value="ROLE_ADMIN" ${user.roleList.some(role => role.role === 'ROLE_ADMIN') ? 'selected' : ''}>ADMIN</option>
            `;
        }
